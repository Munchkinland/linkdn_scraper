...
page = 'https://www.linkedin.com/company/microsoft'
...
html_file = os.path.join(output_dir, f"{company_name}_soup.txt")
with open(html_file, "w+", encoding='utf-8') as t:
...
for container in containers:
    ...
    # Append the post information to the posts_data list
    posts_data.append({
        "Post Text": post_text,
        "Media Link": media_link,
        "Media Type": media_type,
        "Post Date": post_date,
        "Reactions": convert_abbreviated_to_number(post_reactions),
        "Comments": convert_abbreviated_to_number(post_comments),
        "Shares": convert_abbreviated_to_number(post_shares)
    })
...
try:
    ...
    for col in df.columns:
        try:
            ...
            pass
    df.sort_values(by="Reactions", inplace=True, ascending=False)
except Exception as e:
    print(f"Error processing data: {e}")

# Save DataFrame to a CSV file
csv_file = os.path.join(output_dir, f"{company_name}_posts.csv")
df.to_csv(csv_file, index=False)
print(f"Data exported to {csv_file}")

# Export the DataFrame to an Excel file with table formatting
excel_file = os.path.join(output_dir, f"{company_name}_posts.xlsx")
...
# Create a table object for the DataFrame data
tab = Table(displayName="PostsTable", ref=f"A1:{chr(65 + len(df.columns) - 1)}{len(df) + 1}")

# Add a TableStyleInfo to format the table
